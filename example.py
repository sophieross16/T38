...
# NOTES ON RUNNING WITH SM INSTEAD OF MD
print('\nWhen using sm instead of md, the model is smaller and hence less accurate overall:')
print('The model you\'re using has no word vectors loaded, so the result of the Doc.similarity method will be based on the tagger, parser and NER, which may not give useful similarity judgements. This may happen if you\'re using one of the small models, e.g. `en_core_web_sm`, which don\'t ship with word vectors and only use context-sensitive tensors. You can always add your own word vectors, or use one of the larger models instead if available.')
